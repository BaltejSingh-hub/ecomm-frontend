  ...
  useEffect(() => {
    ...
    async function fetchData() {
      const result = await axios({
        ...
        console.log(
          "The useEffect for the specific product detail is being hit"
        );
    ...
    <>
      ...
      <div className="flex justify-center mt-10">
        <div name="shows_images" className="flex flex-col mt-4">
          {product_more_details?.img?.map((e, index) => (
            <div
              key={index}
              name="img_1"
              className={`w-14 h-16 m-4 ${
                imageIndex === index ? "border-1  border-black-200" : ""
              }`}
              onClick={() => {
                SelectImage(index);
              }}
        ...
        <div name="big_image" className="bg-red-300 w-98 h-108 m-4 relative">
          <img
            className="w-98 h-108 shadow-[0_8px_30px_rgb(0,0,0,0.12)]"
            src={product_more_details?.img?.[imageIndex]}
            alt=""
          />
        </div>
        <div className="flex flex-col">
          <div
            name="Product_name"
            className="w-76 h-20 mx-4 mt-4 mb-1 flex items-center"
          >
            <p className="font-medium text-4xl">{product_more_details.title}</p>
          </div>
          <div name="pricing" className="w-36 h-10 ml-4">
            <p className="text-blue-600 font-medium text-2xl mb-3">
              ${product_more_details.price}
            </p>
          </div>
